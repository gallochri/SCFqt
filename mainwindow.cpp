                                       ...
                                       QFile *file)
{
    ...
    query.prepare("INSERT INTO "
                  + dbName +
                  ".testata_listino VALUES "
                  "(?,?,?,?,?,?,?,?,?,?,?,?)");
    int tab[2][12] =
    {
        {0,20,23,34,40,48,56,86,125,128,136,152},
...
void MainWindow::on_import_Metel_triggered()
{
    ...
    listinoDate =
            QDate::fromString(listinoDate,"yyyyMMdd").toString("dd MMMM yyyy");
    ...
    case QMessageBox::Ok:
    {
        ...
        if (lineToTestataListino(db, dbName, file))
        {
            ...
            if (linesToListinoPrezzi(db, dbName, file, lineNr))
            {
                ...
                QString res = QString("%1 sec")
                        .arg(micro_sec/1000000.0, 0, 'f', 2);
                timeImport.setText(QString("Time to import: %1").arg(res));

            } else {
                qDebug() << __func__ << "Problem during import.";
            }
        }
        break;
    }
